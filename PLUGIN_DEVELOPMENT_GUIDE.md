...
| `ReplacesUpstream` | `bool` | Optional. Defaults to `false`. Set on processors whose output is conceptually unrelated to their input — the upstream entries are consumed for context (e.g. their titles become search candidates) and the entries returned are freshly sourced, with their own URLs and lifetimes. The executor excludes the consumed inputs from the pipeline-result counter so they don't dominate the totals as Undecided, and counts the new entries instead. Without this flag, an upstream of 924 titles feeding a search-emitting processor that returned 4 sink-accepted entries would report `total=924, accepted=0`. Set on `discover` today; any future processor whose `Process` returns entries that did not exist on its input slice should set it too |
