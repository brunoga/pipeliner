...
var KnownFields = []FieldMeta{
	...
	{Name: FieldSeriesDoubleEpisode, Type: FieldTypeInt,
		...
		SetBy:       []string{"metainfo_file"}},
	{Name: FieldSeriesName, Type: FieldTypeString,
		Description: "Normalized show name used as the series tracker key (lowercase, separators collapsed)",
		SetBy:       []string{"series_tracker"}},
	{Name: FieldSeriesLifecycle, Type: FieldTypeString,
		Description: "Follow-lifecycle classification for a tracked show: complete (ended, every aired episode tracked), dormant (ended with gaps), active (anything else)",
		SetBy:       []string{"series_lifecycle"},
		KnownValues: []string{SeriesLifecycleComplete, SeriesLifecycleDormant, SeriesLifecycleActive}},
	{Name: FieldSeriesEpisodeCount, Type: FieldTypeInt,
		Description: "Number of downloaded episodes on record for this show in the series tracker",
		SetBy:       []string{"series_tracker"}},
	{Name: FieldSeriesNewestEpisode, Type: FieldTypeString,
		Description: "Highest tracked episode ID for this show (e.g. S03E08)",
		SetBy:       []string{"series_tracker"}},
	{Name: FieldSeriesLastDownloadedAt, Type: FieldTypeTime,
		Description: "Timestamp of the most recent tracked download for this show",
		SetBy:       []string{"series_tracker"}},
	{Name: FieldSeriesInactive, Type: FieldTypeBool,
		Description: "true when the show has been deactivated in the series tracker (series filter rejects its episodes)",
		SetBy:       []string{"series_tracker"}},
	{Name: FieldSeriesAiredEpisodeCount, Type: FieldTypeInt,
		Description: "Number of episodes that have already aired according to TheTVDB",
		SetBy:       []string{"series_lifecycle"}},
	{Name: FieldSeriesMissingEpisodeCount, Type: FieldTypeInt,
		Description: "Number of aired episodes not present in the series tracker",
		SetBy:       []string{"series_lifecycle"}},

	// ── Torrent ──────────────────────────────────────────────────────────
	{Name: FieldTorrentInfoHash, Type: FieldTypeString,
		Description: "SHA-1 info hash (lowercase hex)",
		SetBy:       []string{"rss", "jackett", "metainfo_torrent", "metainfo_magnet", "torrent_session"}},
	...
	{Name: FieldTorrentLeechers, Type: FieldTypeInt,
		...
		SetBy:       []string{"rss", "jackett"}},
	{Name: FieldMovieDigitalRelease, Type: FieldTypeTime,
		Description: "First digital (PVOD/streaming) release date from TMDb",
		SetBy:       []string{"metainfo_tmdb"}},
	{Name: FieldMoviePhysicalRelease, Type: FieldTypeTime,
		Description: "First physical (disc) release date from TMDb",
		SetBy:       []string{"metainfo_tmdb"}},
	{Name: FieldSettled, Type: FieldTypeBool,
		Description: "Release was downloaded after waiting out a settle window",
		SetBy:       []string{"movies", "series"}},
	{Name: FieldSettledRevived, Type: FieldTypeBool,
		Description: "Settled release was rebuilt from the recorded winner because it had left the source feed",
		SetBy:       []string{"movies", "series"}},
	{Name: FieldVideoBitrateMbps, Type: FieldTypeFloat,
		Description: "Implied video bitrate in Mbps (torrent size / runtime) — the quality signal a release name cannot fake",
		SetBy:       []string{"bitrate"}},
	{Name: FieldLibraryQuality, Type: FieldTypeString,
		Description: "Quality of the matching media-library copy (e.g. \"2160p H.265 Atmos Dolby Vision\"), set on entries that matched a library item",
		SetBy:       []string{"library"}},
	...
	// ── Torrent session (torrent_session source) ─────────────────────────
	{Name: FieldTorrentState, Type: FieldTypeString,
		Description: "Normalized download-client torrent state",
		SetBy:       []string{"torrent_session"},
		KnownValues: []string{"downloading", "seeding", "stalled", "paused", "errored", "checking"}},
	{Name: FieldTorrentRatio, Type: FieldTypeFloat,
		Description: "Upload ratio reported by the download client",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentSeedTime, Type: FieldTypeInt64,
		Description: "Cumulative seeding time in seconds",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentAddedAt, Type: FieldTypeTime,
		Description: "When the torrent was added to the client's session",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentLastActivity, Type: FieldTypeTime,
		Description: "Last upload/download activity reported by the client (absent when the client has no record)",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentProgress, Type: FieldTypeFloat,
		Description: "Download completion percentage, 0-100",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentError, Type: FieldTypeString,
		Description: "Client error message; present when torrent_state is \"errored\"",
		SetBy:       []string{"torrent_session"}},
	{Name: FieldTorrentDownloadDir, Type: FieldTypeString,
		Description: "Directory the torrent's data is stored in",
		SetBy:       []string{"torrent_session"}},
