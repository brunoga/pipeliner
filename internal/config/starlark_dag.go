...
func (r *routeHandles) AttrNames() []string {
	names := make([]string, len(r.portOrder))
	copy(names, r.portOrder)
	sort.Strings(names)
	return names
}

// routeBuiltin implements route(upstream, port1="cond1", port2="cond2", ...).
//
// Creates:
//   - one "route" processor node that evaluates all conditions
//   - one "route_selector" node per port, downstream of the route node
//
...
//
// Returns a routeHandles struct with attribute access (routes.port1, etc.).
func (ctx *execContext) routeBuiltin(thread *starlark.Thread, fn *starlark.Builtin, args starlark.Tuple, kwargs []starlark.Tuple) (starlark.Value, error) {
	...
	if len(kwargs) == 0 {
		return nil, fmt.Errorf("route: at least one named port is required (e.g. series=\"...\")")
	}
	...
	// Build ordered rules list and validate port names.
	...
	for _, kv := range kwargs {
		...
		if !ok {
			return nil, fmt.Errorf("route: port %q condition must be a string, got %s", name, kv[1].Type())
		}
	...
	// Create one route_selector node per port.
	handles := &routeHandles{ports: make(map[string]*nodeHandle), portOrder: portOrder}
