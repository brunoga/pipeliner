...
`premiere` is a classifier by construction — every entry it lets through is a
series episode — so it stamps `media_type = "series"` on each processed entry.
Downstream nodes (e.g. `dedup`, `route`, `condition`) can rely on the field
being present as Certain. The `series_*` fields are available downstream
because `metainfo_file` (upstream) already set them.
