...
`title` and `series_episode_id` are declared via `Descriptor.Requires`, so the DAG validator catches pipelines that wire `series` without an upstream metainfo step.
...
|----------|-------|
| Role | `processor` |
| Requires | `title`, `series_episode_id` (declared via `RequireAll`) |
