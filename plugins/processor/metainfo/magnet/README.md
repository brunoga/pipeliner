...
An entry is treated as a magnet when `torrent_link_type = "magnet"` (set by `jackett_search` / `jackett`) or when the URL starts with `magnet:`. Entries with `torrent_link_type = "torrent"` or a non-magnet URL are silently skipped.
