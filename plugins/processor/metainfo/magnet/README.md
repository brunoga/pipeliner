...
An entry is treated as a magnet when `torrent_link_type = "magnet"` (set by `jackett` / `jackett`) or when the URL starts with `magnet:`. Entries with `torrent_link_type = "torrent"` or a non-magnet URL are silently skipped.
...
meta    = process("metainfo_file", upstream=magnet)
flt     = process("condition",  upstream=meta, accept='video_resolution != "" and quality_resolution >= "5"')
